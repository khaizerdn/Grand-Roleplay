...
RegisterNetEvent("hack:checkCooldown", function(group_name)
    local src = source
    print(("Received hack:checkCooldown for %s from source %d"):format(group_name, src))
    local territory = Config.Territories[group_name]
    if not territory then
        print("Error: Territory " .. group_name .. " not found")
        return
    end
    ...
    print(("Cooldown check for %s: now=%d, last=%d, elapsed=%d, cooldownSeconds=%d"):format(group_name, now, last, elapsed, cooldownSeconds))
    if elapsed >= cooldownSeconds then
        print("Cooldown passed, triggering hack:cooldownPassed")
        TriggerClientEvent("hack:cooldownPassed", src, group_name)
    else
...
RegisterNetEvent("hack:setBlipName", function(group_name, blip_name)
    ...
    print(("Hack attempt on %s: now=%d, last=%d, cooldownSeconds=%d"):format(group_name, now, last, cooldownSeconds))
    if cooldownSeconds > 0 then
        local elapsed = now - last
        if elapsed < cooldownSeconds then
            local remaining = cooldownSeconds - elapsed
            print(("Cooldown blocked for %s: elapsed=%d, remaining=%d"):format(group_name, elapsed, remaining))
            TriggerClientEvent("hack:cooldownBlocked", src, remaining)
            return
        else
            print(("Cooldown passed for %s: elapsed=%d"):format(group_name, elapsed))
        end
    end
    ...
    print(("Updated lastHacks[%s] to %d"):format(group_name, now))
